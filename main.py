...
#Llenado
session.run("CREATE (r:region {n_name:'Spain',r_name: 'Catalunya'})")
...
session.run("CREATE (l:LineItem {returnflag:'t',shipdate: '2016/02/06', linestatus: 't', quantity: 2, extendedprice: 50,"
            " discount: 10, "
            "tax: 10})")
...
session.run("CREATE (p:part {p_partyKey: 2, p_mfgr: 'nose', p_size: 10, p_type: 'cosa', ps_supplycost: 10})")
session.run("MATCH(p:part {p_partyKey: 2}) MATCH(s:supplier {s_name:'Roberto'}) CREATE(s)-[w:suppliertopart]->(p)")
...
def queryFour(region, date, date2):
  result = session.run("MATCH (r:region {r_name: %s }) - - > (o:order) - - > (l:LineItem) - - > "
                       "(s:supplier) - - > (r2:region) WHERE o.o_orderdate >= %s AND o.o_orderdate < %s AND r.n_name = r2.n_name RETURN r.n_name, SUM(l.extendedprice*(1-l.discount)) AS revenue "
                       "ORDER BY revenue DESC" % (region, date, date2))
  return result

result = queryOne("'2016/03/06'");
for record in result:
    print(record)

result = queryFour("'Catalunya'", "'2016/01/06'", "'2017/03/06'");
for record in result:
    print(record)
